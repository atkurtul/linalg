...
template <int... n> struct swizz;

template <int r, int c> struct mat;

template <class, int> struct int_vec;
...
template <int...> static constexpr bool is_unique = true;

template <int a, int... t>
static constexpr bool is_unique<a, t...> = ((a != t) && ...) && is_unique<t...>;

template <int...> struct Seqx;

template <int h, int... t> struct Seq0 : Seq0<h - 1, h, t...> {};

template <int... t> struct Seq0<-1, t...> { using Seq = Seqx<t...>; };

template <int n> using Seq = typename Seq0<n - 1>::Seq;

template <class, class> struct Concat;

template <int... a, int... b> struct Concat<Seqx<a...>, Seqx<b...>> {
...
template <int... t> struct Seqx {
  static constexpr inline int n = sizeof...(t);

  using vt = vec<n>;
  using mt = mat<n, n>;

  static constexpr inline void assign(auto &l, vt r) { ((l[t] = r[t]), ...); }
  static constexpr inline vt cast(auto v) { return vt(v[t]...); }
...
#else
  static constexpr inline vt add(vt l, vt r) { return vt(l[t] + r[t]...); }
  static constexpr inline vt sub(vt l, vt r) { return vt(l[t] - r[t]...); }
  static constexpr inline vt mul(vt l, vt r) { return vt(l[t] * r[t]...); }
  static constexpr inline vt div(vt l, vt r) { return vt(l[t] / r[t]...); }
#endif

#undef REQUIRES_NONPACKED

  static constexpr inline vt neg(vt v) { return vt{-v[t]...}; }
  ...
  static constexpr inline mt outer(vt l, vt r) { return mt{l * r[t]...}; }

  static constexpr inline vt clamp(vt v, vt l, vt r) {
    return vt(v[t] < l[t] ? l[t] : v[t] > r[t] ? r[t] : v[t]...);
  }

  static constexpr inline f32 dot(vt l, vt r) { return ((l[t] * r[t]) + ...); }
  ...
  template <int a> static std::ostream &stream(std::ostream &s, mat<a, n> m) {
    return ((s << m[t]), ...) << "\b\n";
  }

  template <int r> static constexpr inline vec<r> mul(mat<r, n> m, vt v) {
    return ((m[t] * v[t]) + ...);
  }

  template <int c> static constexpr inline vt mul(vec<c> v, mat<c, n> m) {
    return vt{dot(m[t], v)...};
  }
  ...
  template <int a> static constexpr inline mat<a, n> mul(mat<a, n> l, f32 r) {
  ...
  template <int a> static constexpr inline mat<n, a> tpos(mat<a, n> m) {
    mat<n, a> re;
    ((Seq<a>::template write<n, t>(re, m[t])), ...);
    return re;
  }

  static constexpr inline void fill(mt &m, f32 x) { ((m[t][t] = x), ...); }
  static constexpr inline void fill(vt &v, f32 x) { ((v[t] = x), ...); }
};
...
template <int n> static constexpr inline int swizz_max<n> = n;

template <int n> static constexpr inline int swizz_min<n> = n;

template <bool s> static constexpr inline f32 set_float_sign(f32 x) {
  (i32 &)x ^= (s << 31);
  return x;
}

template <int x, int y, int z, int w> static constexpr inline int mask() {
...
template <int... n> struct swizz {
...
#else
  constexpr inline operator vt() const { return vt{dat[n]...}; }
#endif
  ...
  constexpr inline f32 &operator[](int i) { return dat[idx[i]]; };
  constexpr inline f32 operator[](int i) const { return dat[idx[i]]; };

  constexpr inline vt operator*(vt r) const { return vt(*this) * r; }
  constexpr inline vt operator/(vt r) const { return vt(*this) / r; }
  constexpr inline vt operator+(vt r) const { return vt(*this) + r; }
  constexpr inline vt operator-(vt r) const { return vt(*this) - r; }
  constexpr inline vt operator*(f32 r) const { return vt(*this) * r; }
  constexpr inline vt operator/(f32 r) const { return vt(*this) / r; }
  constexpr inline vt operator-() const { return -vt(*this); }
  constexpr inline friend vt operator*(f32 l, swizz r) { return l * vt(r); }
  constexpr inline friend vt operator/(f32 l, swizz r) { return l / vt(r); }
  constexpr inline swizz &operator*=(vt r) { return *this = vt(*this) * r; }
  constexpr inline swizz &operator/=(vt r) { return *this = vt(*this) / r; }
  constexpr inline swizz &operator+=(vt r) { return *this = vt(*this) + r; }
  constexpr inline swizz &operator-=(vt r) { return *this = vt(*this) - r; }
  constexpr inline swizz &operator*=(f32 r) { return *this = vt(*this) * r; }
  constexpr inline swizz &operator/=(f32 r) { return *this = vt(*this) / r; }
  constexpr inline f32 dot(vt r) const { return vt(*this).dot(r); }
  constexpr inline f32 len() const { return vt(*this).len(); }
  constexpr inline f32 len2() const { return vt(*this).len2(); }
  constexpr inline mat<ns, ns> outer(vt r) const { return vt(*this).outer(r); }
  constexpr inline vt norm() const { return vt(*this).norm(); }
  constexpr inline swizz &normalize() { return *this /= len(); }
...
template <int n> struct vec_data {
  f32 dat[n];
...
template <int r, int c> struct mat_data { vec<r> dat[c]; };

template <int r> struct mat_data<r, 2> {
  union {
    vec<r> dat[2];
    struct {
      vec<r> x, y;
...
template <int r> struct mat_data<r, 3> {
  union {
    vec<r> dat[3];
    struct {
      vec<r> x, y, z;
...
template <int r> struct mat_data<r, 4> {
  union {
    vec<r> dat[4];
    struct {
      vec<r> x, y, z, w;
...
template <class T, int n> struct int_vec_data { T dat[n]; };

template <class T> struct int_vec_data<T, 2> {
  union {
    T dat[2];
    T x, y;
  };
...
template <class T> struct int_vec_data<T, 3> {
  union {
    T dat[3];
    T x, y, z;
  };
...
template <class T> struct int_vec_data<T, 4> {
  union {
    T dat[4];
    T x, y, z, w;
  };
...
template <class T, int n> struct int_vec : int_vec_data<T, n> {
  using int_vec_data<T, n>::dat;

  constexpr inline bool operator==(int_vec const &) const = default;
  constexpr inline T operator[](int i) const { return dat[i]; }
  constexpr inline T &operator[](int i) { return dat[i]; }
  constexpr inline operator vec<n>() const { return Seq<n>::cast(*this); }
};

template <class U, int n> struct ctor_arg {
...
template <int n> struct vec : vec_data<n> {
  ...
  constexpr inline explicit vec(f32 x) { Seq<n>::fill(*this, x); }
...
#if __x86_64 && USE_SSE
  constexpr inline vec(__m128 v) requires(n == 4) { this->xmm = v; }
#endif
  ...
  constexpr inline f32 &operator[](int i) { return dat[i]; };
  constexpr inline f32 operator[](int i) const { return dat[i]; };
  constexpr inline f32 *begin() { return dat; }
  constexpr inline f32 *end() { return dat + n; }
  constexpr inline const f32 *begin() const { return dat; }
  constexpr inline const f32 *end() const { return dat + n; }
  constexpr inline vec operator-() const { return Seq<n>::neg(*this); }
  ...
  constexpr inline vec operator*(vec r) const { return mul(*this, r); }
  constexpr inline vec operator/(vec r) const { return div(*this, r); }
  constexpr inline vec operator+(vec r) const { return add(*this, r); }
  constexpr inline vec operator-(vec r) const { return sub(*this, r); }
  constexpr inline vec operator*(f32 r) const { return mul(*this, vec(r)); }
  constexpr inline vec operator/(f32 r) const { return div(*this, vec(r)); }
  constexpr inline friend vec operator*(f32 l, vec r) { return mul(vec(l), r); }
  constexpr inline friend vec operator/(f32 l, vec r) { return div(vec(l), r); }

  constexpr inline vec &operator*=(vec r) { return *this = *this * r; }
  constexpr inline vec &operator/=(vec r) { return *this = *this / r; }
  constexpr inline vec &operator+=(vec r) { return *this = *this + r; }
  constexpr inline vec &operator-=(vec r) { return *this = *this - r; }
  constexpr inline vec &operator*=(f32 r) { return *this = *this * r; }
  constexpr inline vec &operator/=(f32 r) { return *this = *this / r; }
  constexpr inline f32 dot(vec r) const { return Seq<n>::dot(*this, r); }

  constexpr inline f32 alternating_dot(vec r) const {
    return Seq<n>::alternating_dot(*this, r);
  }

  constexpr inline f32 len() const { return sqrtf(len2()); }
  constexpr inline f32 len2() const { return dot(*this); }
  constexpr inline mat<n, n> outer(vec r) const {
    return Seq<n>::outer(*this, r);
  }
  constexpr inline vec norm() const { return *this / len(); }
  constexpr inline vec &normalize() { return *this /= len(); }
...
template <int r, int c> struct mat : mat_data<r, c> {
  using base = mat_data<r, c>;
  using base::dat;

  constexpr inline vec<r> &operator[](int i) { return dat[i]; };
  constexpr inline vec<r> const &operator[](int i) const { return dat[i]; };
  ...
  constexpr inline explicit mat(f32 x) requires(r == c) : base{} {
    Seq<r>::fill(*this, x);
  }
  ...
  template <int n> constexpr inline mat<r, n> operator*(mat<c, n> m) const {
    return Seq<n>::mul(*this, m);
  }

  constexpr inline mat operator*(f32 x) const { return Seq<c>::mul(*this, x); }
  constexpr inline friend mat operator*(f32 x, mat m) { return m * x; }
  constexpr inline mat operator+(mat m) const { return Seq<c>::add(*this, m); }
  constexpr inline mat operator-(mat m) const { return Seq<c>::sub(*this, m); }
  constexpr inline mat<c, r> tpos() const { return Seq<c>::tpos(*this); }
...
constexpr inline mat4 perspective(f32 fov, f32 ar, f32 n, f32 f) {
  mat4 m = {};
...
constexpr inline mat3 axis_angle(vec3 axis, f32 angle) {
  axis.normalize();
  f32 c = cosf(angle);
...
struct quat : vec_data<4> {
  constexpr inline quat() { xyzw = vec4(0, 0, 0, 1); }
  constexpr inline quat(vec3 axis) { xyzw = vec4(axis, 0); }

  constexpr inline quat(vec3 axis, f32 angle) {
    axis.normalize();
    angle *= 0.5f;
    w = cosf(angle);
    xyz = sinf(angle) * axis;
  ...
  explicit constexpr inline quat(vec4 v) { xyzw = v.xyzw; }

  constexpr inline f32 dot(quat q) const { return xyzw.dot(q.xyzw); }
  ...
  constexpr inline quat operator~() const { return quat(vec4(-xyz, w)); }
};
